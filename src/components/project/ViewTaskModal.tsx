            ...
            <Tag
              style={{
                backgroundColor: columns
                  .find((c) => c.id === viewTask?.status)
                  ?.color?.slice(0, 7)
                  ? `${columns.find((c) => c.id === viewTask?.status)?.color?.slice(0, 7)}40`
                  : "#94a3b8",
              }}
          ...
          <div className="flex flex-col">
            ...
            {isEditingDescription ? (
              ...
              <div className="border border-gray-200 rounded-lg p-2 mb-1 max-h-[60vh] overflow-y-auto">
                <div
                  className="[&>p]:mb-3 [&>h1]:text-2xl [&>h1]:font-bold [&>h2]:text-xl [&>h2]:font-bold [&>h3]:text-lg [&>h3]:font-bold [&>ul]:mb-3 [&>ul]:list-disc [&>ul]:pl-5 [&>ol]:mb-3 [&>ol]:list-decimal [&>ol]:pl-5 [&_img]:max-w-full [&_img]:rounded-md [&_img]:cursor-zoom-in [&_a]:text-blue-600 [&_a]:underline [&_a]:hover:text-blue-800"
                  onClick={handleDescriptionImageClick}
                  dangerouslySetInnerHTML={{ __html: viewTask.description }}
                />
                <Image
                  src={previewImageSrc}
                  alt="task-description-image-preview"
                  style={{ display: "none" }}
                  preview={{
